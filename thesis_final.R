...
# All items, GY (Growth rate same period previous year), Q (Quarterly Frequency)
...
# I do have data_all.xlsx, perhaps I can just import this instead of running all this code in the future
...
# compare to other measures/records
...
data_final <- data_gap %>%
  ...
  mutate(short_lag = lag(short_rate, n=1, default=NA), 
         dshort_rate = short_rate - lag(short_rate, n=1, default=NA))
...
for (i in countries_final){
  ...
  # tv_reg <- tvAR(data_temp$short_rate, p=1, exogen = cbind(data_temp$cpi, data_temp$gap), bw=NULL)
  ...
  # tv_reg <- tvLM(dshort_rate ~ cpi + gap, data = data_temp, bw=NULL)
  # data_temp$LOCATION <- i
  # data_coef[data_coef$LOCATION==i,]$binf <- c(NA,tv_reg$coefficients[,2])
  # data_coef[data_coef$LOCATION==i,]$bgdp <- c(NA,tv_reg$coefficients[,3])
}
...
# Define the shaded recession periods
...
ggplot(data_check) +
  ...
  ggtitle("Consumer Price Index, United States")+
  xlab("Time") +
  ylab("% Change y/y") +
  geom_rect(data=recessions, aes(xmin=start, xmax=end, ymin=-Inf, ymax=Inf), fill="gray", alpha=0.2) +
  theme_classic()
...
# CPI
cpi_summary <- data_coef %>% group_by(LOCATION) %>% summarize(cpi = mean(cpi, na.rm=T))
summary(cpi_summary)
...
global_recession_dates <- data.frame(start = as.Date(c("1973-01-01", "1980-01-01", "1990-07-01", "2008-12-01", "2020-02-20")),
...
data_coef$Date <- as.Date(as.yearqtr(data_gap$obsTime, format = "%Y-Q%q"))
ggplot(data_coef, aes(x=Date,y=brate, color = LOCATION)) + geom_line() +
  ggtitle("Global Lagged Short-Rate Coefficients") +
  xlab("Time") +
  ylab("Value") +
  theme_classic()
...
cpi_regions <- inner_join(cpi_summary, country.names, by="LOCATION") %>% rename(value=cpi)
map_binf <- country_choropleth(cpi_regions,num_colors=1, title="Average Inflation By Country")
map_binf + scale_fill_continuous(low="#eff3ff", high="#084594", na.value="gray")

# Inflation Coefficient By Country
country.names <- country.map %>% group_by(iso_a3) %>% filter(row_number()==1) %>% select(iso_a3,region) %>% rename(LOCATION=iso_a3)
binf_regions <- inner_join(binf_med, country.names, by="LOCATION") %>% rename(value=med_binf)
map_binf <- country_choropleth(binf_regions,num_colors=1, title="Median Inflation Coefficients By Country", legend="Beta")
map_binf + scale_fill_continuous(low="#eff3ff", high="#084594", na.value="gray")


#### post-mea tings ####
# import data_coef bc the short-rate got fucked up
data_coef <- import('data_coef.xlsx')

# replicate binf for bgdp
# Gap coef data frame by time period
data_bgdp <- data_coef %>% 
  filter(Date > as.Date('1960-01-01')) %>%
  group_by(Date) %>% 
  summarize(bgdp_med = median(bgdp, na.rm=T), bgdp_sd = 1/sd(bgdp,na.rm=T), N = sum(!is.na(gap)))
# Gap coef data frame by country
bgdp_summary <- data_coef %>% group_by(LOCATION) %>% summarize(mean_bgdp=mean(bgdp, na.rm=T), sd_bgdp = round(sd(bgdp, na.rm=T),2))


# create filtered data.frame for median plot
data_small <- data_bgdp %>% filter(Date > as.Date("1961-01-01"))

# plot median bgdp across time
ggplot() +
    geom_line(aes(x = data_small$Date, y = data_small$bgdp_med),color="turquoise4") +
  geom_rect(data = global_recessions, aes(xmin = xmin, xmax = xmax, ymin = -Inf, ymax = Inf), fill = "gray80", alpha = 0.5) +
    ggtitle("Median Output Gap Coefficient") +
    xlab("Time") +
    ylab("Value") +
    theme_classic()
...
# plot ALL bgdp
data_coef$Date <- as.Date(as.yearqtr(data_gap$obsTime, format = "%Y-Q%q"))
ggplot(data_coef, aes(x=Date,y=bgdp, color = LOCATION)) + geom_line() +
  ggtitle("Global Output Gap Coefficients") +
  xlab("Time") +
  ylab("Value") +
  theme_classic()

# plot bgdp ex. outliers
data_coef$Date <- as.Date(as.yearqtr(data_gap$obsTime, format = "%Y-Q%q"))
ggplot(data_coef, aes(x=Date,y=bgdp, color = LOCATION)) + geom_line() +
  ggtitle("Global Output Gap Coefficients") +
  xlab("Time") +
...
# plot ALL sd bgdp across time + subperiod markers
ggplot() +
  geom_line(aes(x=data_bgdp$Date,y=data_bgdp$bgdp_sd),color="deeppink3") +
  ggtitle("Convergence of Output Gap Responses") +
  xlab("Time") +
  ylab("Inverse of Standard Deviation") +
  theme_classic()
